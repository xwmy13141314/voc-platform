...
| 当前版本 | v1.2.0 已交付（v2.0 升级 Phase 1+2：质量过滤 + 痛点聚类），本 PRD 为理论升级后的目标终态 |
| 项目方 | RugOne 团队 |
| 创建日期 | 2026-08-18 |
| 最近更新 | 2026-09-01 |
...
RugOne 团队专注三防手机（rugged phone）产品线，面向户外、工业、极端环境用户。这类用户高度聚集在 YouTube、Reddit 等海外社媒平台，评论中蕴含大量关于防护性能、按键手感、电池续航、屏幕亮度等硬件维度的真实使用反馈。
...
三防手机市场年规模约 30 亿美元，年增长率 6-8% `[To be confirmed — 需补充权威市场报告引用]`。核心用户群体为户外工作者、工业巡检人员、军警消防人员，以及极端环境爱好者。这类用户对硬件参数敏感度高，且在社媒平台上有较强的表达意愿，评论数据质量远高于普通消费电子产品。
...
**场景一：改良需求挖掘（PM 日常）**
PM 打开看板，选择"Blackview"品牌 + "button"痛点标签，查看按键相关负面评论聚类。系统展示四元组分析结果，PM 发现多条评论提到"戴手套按不准侧键"。点击展开原声追溯链接，确认评论真实性。系统已将该聚类与竞品规格库对比，提示"竞品 Doogee 侧键行程 1.2mm，本品牌 0.5mm"。PM 一键导出改良提案，提交研发。

**场景二：竞品对标分析（PM/市场）**
PM 选择三个竞品（Blackview、Doogee、Ulefone），在 Spec-Pain Matrix 中查看重量维度的回归曲线。系统显示"机身重量超过 380g 后，'手感笨重'负面声量呈指数级上升"，而 Blackview 当前主力机型 420g，正好落在临界点之上。PM 据此向研发提出减重至 380g 以内的改良目标。
...
| F23 | 改良提案 — 建议规格 | 基于竞品规格库与回归分析，给出具体参数建议（如"按键行程 0.5mm → 1.2mm，对标 Doogee V40 Pro"）。 |
...
用户选择目标平台和品牌，输入搜索关键词（可选，缺省使用品牌默认关键词）。关键词以逗号分隔，每段独立搜索。选择"全部品牌"时使用 OR 查询（如 `Blackview OR Doogee`）以减少请求次数。
...
> 竞品 Doogee V40 Pro：360g（低于临界值，负面声量 8%）。
...
2. 建议规格（F23）：基于竞品规格库与回归分析，给出具体参数建议。如"将按键行程从 0.5mm 调整至 1.2mm（对标 Doogee V40 Pro），增加硬件级防误触按键锁"。
...
- 应用图标：多尺寸 ICO（16/32/48/64/128/256px），主题为三防手机 + 数据洞察
